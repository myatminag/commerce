...
export class CategoryService {
  ...
  async createMainCategory(dto: CreateMainCategoryDto) {
    ...
    const category = await this.prismaService.extend.category.findFirst({
      where: {
        slug,
      },
    });
    ...
    return await this.prismaService.extend.category.create({
      data: {
        ...dto,
        slug,
  ...
  async createSubCategory(dto: CreateSubCategoryDto) {
    ...
    const [mainCategory, existingSlugs] = await Promise.all([
      this.prismaService.extend.category.findUnique({
        where: { id: dto.parent_id },
        select: { id: true },
      }),
      this.prismaService.extend.category.findMany({
        where: { slug: { in: slugs } },
        select: { name: true },
      }),
    ]);
    ...
    return await this.prismaService.extend.category.createMany({
      data: subCategories,
      skipDuplicates: true,
    });
  }
  ...
  async getAllCategories({ limit, offset, search }: QueryParamsDto) {
    ...
    const [count, categories] = await this.prismaService.$transaction([
      this.prismaService.extend.category.count({
  ...
  async updateMainCategory(slug: string, dto: UpdateMainCategoryDto) {
    ...
    return await this.prismaService.extend.category.update({
  ...
  async updateSubCategory(slug: string, dto: UpdateSubCategoryDto) {
    const [subCategory, mainCategory] = await Promise.all([
      this.prismaService.extend.category.findFirst({
        where: { slug },
        select: { id: true },
      }),
      this.prismaService.extend.category.findUnique({
  ...
  async deleteCategory(slug: string) {
    ...
    await this.prismaService.extend.category.delete({
      where: { id: category.id },
    });
  ...
  async deleteMainCategories(dto: DeleteCategoriesDto) {
    const categories = await this.prismaService.extend.category.deleteMany({
      where: {
        slug: { in: dto.slugs },
        parent_id: null,
      },
