  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "text-sidebar-foreground w-(--sidebar-width) flex h-full flex-col bg-white",
          className,
        )}
        {...props}
      >
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <MenuIcon className="size-7 text-neutral-950" />
...
function SidebarMenu({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      data-sidebar="menu"
      className={cn("flex w-full min-w-0 flex-col gap-x-1 gap-y-2", className)}
      {...props}
    />
  );
}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] focus-visible:ring-2 active:bg-brand-600 active:text-sidebar-primary-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium group-data-[collapsible=icon]:!h-10 group-data-[collapsible=icon]:!w-[47px] group-data-[collapsible=icon]:!p-3.5 [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:text-[#64716F] [&>svg]:group-data-[state=open]/collapsible:text-white [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "hover:bg-sidebar-accent hover:text-sidebar-accent-foreground group-data-[state=open]/collapsible:hover:bg-brand-600 group-data-[state=open]/collapsible:hover:text-sidebar-primary-foreground",
        outline:
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-sidebar-border mx-3.5 mt-2 flex min-w-0 translate-x-px flex-col gap-x-1 gap-y-3 border-l px-2.5 py-0.5",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
